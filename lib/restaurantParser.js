    ...
    //dwr.engine._remoteHandleCallback('0','0',[s0,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15]);
        ...
        for (var x = 1; x <= raw_array.length; x += 2) {
            ...
            tempInspection = {
                ...
                grade_date: (data['gradeDate']) ? new Date(parseInt(data['gradeDate'].replace('new Date(','').replace(')',''), 10)) : '',
...
//dwr.engine._remoteHandleCallback('2','0',{lat:40.8912353515625,lng:-73.85856628417969});
        ...
        var re = /\'/g;
        var restaurant_camis = parseInt(raw_data[0].split(',')[1].replace(re,''), 10);
        var raw_geo_data = (raw_data[1]) ? raw_data[1].replace('{','').replace('lat:','').replace('lng:','').replace('}','').replace('\r\n','').split(',') : [];
        var data = {};
        ...
        data.camis = restaurant_camis;
        data.geoData = {
            lat: parseFloat(raw_geo_data[0]),
            lon: parseFloat(raw_geo_data[1])
        };

        data.GeoJSON = {
            type: "Point",
            coordinates: [parseFloat(raw_geo_data[0]), parseFloat(raw_geo_data[1])]
        };
...
//s10.neighName="Battery Park City";s10.resZipCode="10280_10282";
    ...
    return {
        parseInspectionForRestaurant: parseInspectionForRestaurant,
        parseDetailsForInspection: parseDetailsForInspection,
        parseRestaurantBatch: parseRestaurantBatch,
        parseGeoDataBody: parseGeoDataBody,
        parseTotalResponse: parseTotalResponse,
        parseNeighborhoodResponse: parseNeighborhoodResponse
    }
}();
