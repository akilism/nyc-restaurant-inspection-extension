...
var restaurantConnector = function () {
    ...
    var getRestaurantBatch = function (batch, batch_size, callback) {
    ...
    var getTotalCount = function (callback_function) {
    ...
    var getNeighborhoods = function (callback_function) {
    ...
    var getGeoData = function(camis, callback_function) {
        var url ='http://a816-restaurantinspection.nyc.gov/RestaurantInspection/dwr/call/plaincall/RestaurantSpringService.getRestaurantDetailsMapInfo.dwr';
        var payload = {
            callCount: 1,
            ...
            'c0-id': camis,
            'c0-param0': 'string:' + camis ,
            batchId: 1
        };
        //using the camis as the id to tie the async response back to the correct restaurant.
        connector.ajaxConnector.setConnection(url, connector.payloadToString(payload), callback_function);
        connector.ajaxConnector.connect();
    };

    var getInspections = function(camis, callback_function) {
        ...
        var payload = {
            ...
            'c0-id': camis,
            'c0-param0': 'string:restaurantInspectionPK.camis%20%3A%20' + camis ,
            batchId: 1
        };
        //using the camis as the id to tie the async response back to the correct restaurant.
        connector.ajaxConnector.setConnection(url, connector.payloadToString(payload), callback_function);
    ...
    var getViolationDetails = function (camis, date, program, callback_function) {
        date = urlEncode(date);
        ...
        var payload = {
            ...
            'c0-id': camis,
            'c0-param0': 'string:restaurantInspectionPK.camis%20%40' + camis + '%0ArestaurantInspectionPK.inspectionDate%20%40' + date + '%0ArestaurantInspectionPK.program%20%40' + program,
            batchId: 1
        };
        //using the camis as the id to tie the async response back to the correct restaurant.
        connector.ajaxConnector.setConnection(url, connector.payloadToString(payload), callback_function);
    ...
    return {
        getRestaurantBatch : getRestaurantBatch,
